...
void help() {
  std::cerr << "Usage: root2hdf5 fileLocation1, [fileLocation2, [...]], treeLocation" << std::endl << std::endl
            ...
            << "HDF5 is streamed to standard output and can be redirected to a file." << std::endl << std::endl
            << "Options:" << std::endl
            ...
            << "  --mode=MODE               What to write to standard output: \"hdf5\" (HDF5 file, default), \"json\" (one JSON" << std::endl
            << "                            object per line), \"schema\" (HDF5 schema only), \"repr\" (ROOT representation only)," << std::endl
            << "                            or \"c++\" (show C++ code that would be generated from streamers with --inferTypes)." << std::endl
            << "  --codec=CODEC             Codec for compressing the HDF5 output; may be \"null\" (uncompressed, default)," << std::endl
            << "                            \"deflate\", \"snappy\", \"lzma\", depending on libraries installed on your system." << std::endl
            << "  --block=SIZE              HDF5 block size in KB (default is 64); if too small, no output will be produced." << std::endl
...
int main(int argc, char **argv) {
  ...
  for (int fileIndex = 0;  fileIndex < fileLocations.size();  fileIndex++) {
    ...
    if (treeWalker != nullptr) {
      ...
      treeWalker = new TreeWalker(url, treeLocation, schemaName, ns, false);
    ...
    // print out HDF5 bytes (with an "Obj" header)
    if (mode == std::string("hdf5")) {
      ...
      //if (!treeWalker->printHDF5HeaderOnce(codec, blockKB * 1024)) return -1;
      do {
        if (end != NA  &&  currentEntry >= end) {
        //  treeWalker->closeHDF5();
          return 0;
        }

        std::cout << treeWalker->numEntriesInCurrentTree() << "\n";
      //  if (!treeWalker->printHDF5()) {
         // treeWalker->closeHDF5();
        ///  return -1;
      //  }
...
/*
    // print the HDF5 dump and exit
    else if (mode == std::string("schema")) {
      std::cout << treeWalker->hdf5Schema() << std::endl;
      return 0;
    }
    else {
