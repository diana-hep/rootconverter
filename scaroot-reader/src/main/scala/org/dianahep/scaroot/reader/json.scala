...
/** Provides support for parsing and stringifying JSON.
  * 
  * This module is a new implementation; it does not depend on packages such as Jackson.
  */
package object json {
...
package json {
  ...
  object Json {
    /** Parses a JSON string into [[org.dianahep.scaroot.reader.json.Json]] objects.
      *
  ...
  /** Concrete class for JSON strings.
    * 
    * '''Note:''' the strings `"-inf"`, `"inf"`, and `"nan"` can be interpreted as numbers by [[org.dianahep.scaroot.reader.json.JsonNumber]]. If `JsonNumber` parsing is attempted ''before'' `JsonString` in an `orElse` chain, these three values would become numbers; otherwise they would become strings. Standard parsing (provided by the [[org.dianahep.scaroot.reader.json.JsonPrimitive]] and [[org.dianahep.scaroot.reader.json.Json]] objects) attempts to interpret them as numbers first.
    */
