...
package reader {
  ...
  class RootTreeReloadableIterator[TYPE](var fileLocation: String,
                                         var num: Int,
                                         var of: Int,
                                         treeLocation: String,
                                         libs: Seq[String] = Nil,
    ...
    // Limit user access to incrementIndex, which should preserve interrelationships.
    ...
    val schema: SchemaClass = {
      treeWalker = RootReaderCPPLibrary.newTreeWalker(fileLocation, treeLocation, "")
    ...
    private var entriesInFile = -1L
    private var start = 0L
    private var end = -1L

    def reload(fileLocation: String, num: Int, of: Int) {
      if (num >= of)
        throw new IllegalArgumentException(s"The num ($num, 0-indexed number in sequence) must be less than the of ($of, size of sequence).")
      if (this.fileLocation != fileLocation) {
        this.fileLocation = fileLocation
        RootReaderCPPLibrary.reset(treeWalker, fileLocation)
      }
      entriesInFile = RootReaderCPPLibrary.numEntriesInCurrentTree(treeWalker)
      this.num = num
      this.of = of
      start = (num * entriesInFile) / of
      end = ((num + 1) * entriesInFile) / of

      done = (RootReaderCPPLibrary.next(treeWalker) == 0)
    }

    reload(fileLocation, num, of)

    def index = entryIndex
    def progress = (entryIndex - start).toDouble / (end - start).toDouble
    ...
    private def thisMicroBatchSize =
      if (entriesInFile - entryIndex > microBatchSize)
        microBatchSize
      else
        (entriesInFile - entryIndex).toInt

    def hasNext = !done
    def next() = {
      if (done)
        throw new RuntimeException("next() called on empty RootTreeIterator (reload to run over more data)")

      if (microBatchIndex == 0) {
        // Set the status byte to 1 (writing) and let C++ write to the buffer.
        statusByte = 1
        buffer.setByte(0, statusByte)
        RootReaderCPPLibrary.copyToBuffer(treeWalker, entryIndex, thisMicroBatchSize, buffer, bufferSize)
        byteBuffer.rewind()
      }
      ...
      // Increment the counter and see if it's time to stop.
      incrementIndex()

      out
    }
  }
  object RootTreeReloadableIterator {
    def apply[TYPE](fileLocation: String,
                    num: Int,
                    of: Int,
                    treeLocation: String,
                    libs: Seq[String] = Nil,
      ...
      new RootTreeReloadableIterator[TYPE](fileLocation, num, of, treeLocation, libs, myclasses, microBatchSize)
  }
